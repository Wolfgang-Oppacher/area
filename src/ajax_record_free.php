<?php
    // -----------------------------------------------------
    // module: sets record free from user ownership
    // status: v6 finished
    // -----------------------------------------------------
    ...
    try
        {
        // prepare sql command
        $stmt = $dbh -> prepare ("UPDATE `".$gbl_db_data[$dbix]['dbname']."` SET `".$ownerid_key."`=0 WHERE `id`=:recordid");
    ...
    editors_addrecord ($dbh, $gbl_actuser['id'], $dbix, $recordid, 'freerecord', '', '');
